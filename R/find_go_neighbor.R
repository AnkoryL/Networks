#' Find the neighbors of the target gene in GO annotation
#'
#' @param gene_name Path to gene list (CSV or TXT).
...
find_go_neighbor <- function(
  ...
  if (is.null(filtered_data) || nrow(filtered_data) < 2) stop(error_messages$no_genes_after_filtering)
	output_list<-list(go_df,annotated,annotated_genes)
	return(output_list)
